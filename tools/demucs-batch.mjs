...
const songs = global.window.SONGS.filter(
  (s) => s.id.includes(filter) && s.text && !(s.notes && s.notes.length)
);
console.log(`${songs.length} "${filter}" chord songs for Demucs treatment\n`);

const STEMS = `${SCR}/stems`;
...
for (const s of songs) {
  ...
  const wav = `${AUD}/${id}.wav`, sep = `${SCR}/sep_${id}`, stem = `${STEMS}/${id}.wav`;
  const ch = `${SCR}/ch_${id}.json`, tr = `${SCR}/tr_${id}.json`;
  const cleanTmp = () => { try { fs.rmSync(wav, { force: true }); fs.rmSync(sep, { recursive: true, force: true }); } catch {} };
  try {
    if (!fs.existsSync(stem)) {                          // separate once, then cache the stem
      ...
      cleanTmp();                                         // keep only the cached stem
    }
  ...
  } catch (e) { cleanTmp(); fail++; console.log(`✗ ${s.title} — ${String(e.message).split("\n")[0].slice(0, 55)}`); }
}
